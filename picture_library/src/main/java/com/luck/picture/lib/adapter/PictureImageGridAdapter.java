...
public class PictureImageGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public PictureImageGridAdapter(Context context, PictureSelectionConfig config) {
        this.videoMaxSecond = config.videoMaxSecond;
                ...
                contentHolder.ll_check.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (!new File(newPath).exists()) {
                            ToastManager.show(context, PictureMimeType.s(context, mediaMimeType));
                            return;
                        }
                        if (mediaMimeType == PictureConfig.TYPE_VIDEO) {
                            if (duration > videoMaxSecond) {
                                if (videoMaxSecond < 60 * 1000) {
                                    ToastManager.show(context, "请选择小于" + videoMaxSecond / 1000 + "s的视频");
                                } else {
                                    ToastManager.show(context, "请选择小于" + videoMaxSecond / (60 * 1000) + "分钟的视频");
                                }
                                return;
                            }
                        }
    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        if (!TextUtils.isEmpty(pictureType)) {
            ...
            if (!toEqual) {
                ToastManager.show(context, context.getString(R.string.picture_rule));
                return;
            }
        }
